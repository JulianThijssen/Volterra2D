...
public class ShapeLoader {
	public static int getQuad() {
		...
		for (int i = 0; i < elements.length; i++) {
			...
			vertexBuffer.put(vertices[index * 3 + 0]);
			vertexBuffer.put(vertices[index * 3 + 1]);
			vertexBuffer.put(vertices[index * 3 + 2]);
		}
		vertexBuffer.flip();
		...
		for (int i = 0; i < elements.length; i++) {
			int index = elements[i];
			texCoordBuffer.put(texCoords[index * 2 + 0]);
			texCoordBuffer.put(1 - texCoords[index * 2 + 1]);
		}
		texCoordBuffer.flip();
